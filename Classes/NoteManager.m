...
- (void)addLocation:(CLLocation *)locationNow
{
    ...
    [note setAltitude:@(locationNow.altitude)];
    NSLog(@"Altitude: %f", [note.altitude doubleValue]);
    
    [note setLatitude:@(locationNow.coordinate.latitude)];
    NSLog(@"Latitude: %f", [note.latitude doubleValue]);
    
    [note setLongitude:@(locationNow.coordinate.longitude)];
    NSLog(@"Longitude: %f", [note.longitude doubleValue]);
    
    [note setSpeed:@(locationNow.speed)];
    NSLog(@"Speed: %f", [note.speed doubleValue]);
    
    [note setHAccuracy:@(locationNow.horizontalAccuracy)];
    NSLog(@"HAccuracy: %f", [note.hAccuracy doubleValue]);
    
    [note setVAccuracy:@(locationNow.verticalAccuracy)];
...
- (void)saveNote
{
	...
	NSDictionary *postVars = @{@"note": noteJson,
							  @"version": [NSString stringWithFormat:@"%d", kSaveNoteProtocolVersion]};
	// create save request
...
{
	NSLog(@"%ld bytesWritten, %ld totalBytesWritten, %d totalBytesExpectedToWrite",
